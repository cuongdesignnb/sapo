...
// Filter state
const concern = ref(props.filters.concern || "product");
...
const applyFilter = () => {
    ...
    if (deliveryFrom.value) params.delivery_from = deliveryFrom.value;
    if (deliveryTo.value) params.delivery_to = deliveryTo.value;
    router.get("/reports/orders", params, { preserveState: true });
};

// Auto-apply when key filters change
watch([concern, period, branchId, status], () => {
    applyFilter();
});
...
const isHorizontal = computed(() => props.chartData?.type === "horizontal_bar");
    ...
    <AppLayout>
        <div class="flex h-full min-h-[calc(100vh-56px)]">
            ...
            <aside class="w-[200px] bg-white border-r border-gray-200 p-4 flex-shrink-0 overflow-y-auto print:hidden">
                <h2 class="text-sm font-bold text-gray-800 mb-4">Báo cáo đặt hàng</h2>
                <div class="mb-4">
                    <label class="text-xs text-gray-500 font-medium mb-1.5 block">Kiểu hiển thị</label>
                    <div class="flex gap-1">
                        <button @click="switchView('chart')" class="px-3 py-1.5 text-xs rounded transition-colors" :class="viewMode === 'chart' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'">Biểu đồ</button>
                        <button @click="switchView('report')" class="px-3 py-1.5 text-xs rounded transition-colors" :class="viewMode === 'report' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'">Báo cáo</button>
                    </div>
                </div>
                <div class="mb-4">
                ...
                <div class="mb-4">
                    <label class="text-xs text-gray-500 font-medium mb-1.5 block">Chi nhánh xử lý</label>
                ...
                <div class="mb-4">
                    <label class="text-xs text-gray-500 font-medium mb-1.5 block">Thời gian đặt hàng •</label>
                ...
                <div class="mb-4">
                    <label class="text-xs text-gray-500 font-medium mb-1.5 block">Thời gian giao hàng •</label>
                    <div class="space-y-1.5">
                        <input type="date" v-model="deliveryFrom" class="w-full text-xs border border-gray-300 rounded px-2 py-1.5" />
                        <input type="date" v-model="deliveryTo" class="w-full text-xs border border-gray-300 rounded px-2 py-1.5" />
                        <button @click="applyFilter" class="w-full text-xs bg-blue-50 text-blue-600 border border-blue-200 rounded py-1.5 hover:bg-blue-100">Áp dụng</button>
                    </div>
                </div>
                <div class="mb-4">
                    <label class="text-xs text-gray-500 font-medium mb-1.5 block">Trạng thái</label>
                    <select v-model="status" class="w-full text-sm border border-gray-300 rounded px-2 py-1.5">
            ...
            <main class="flex-1 bg-gray-50 overflow-auto">
                ...
                <template v-if="viewMode === 'chart'">
                    <div class="p-6">
                        <div class="text-center mb-4"><h2 class="text-lg font-semibold text-gray-700">{{ chartData?.title || 'Báo cáo đặt hàng' }}</h2></div>
                        <div class="bg-white rounded-lg border border-gray-200 p-6">
                            <div v-if="chartData?.labels?.length > 0" :style="{ height: isHorizontal ? Math.max(300, chartData.labels.length * 50) + 'px' : '400px' }">
                                <Bar :data="barChartData" :options="barChartOptions" />
                            </div>
                            <div v-else class="h-[400px] flex items-center justify-center text-gray-400">
                                <div class="text-center"><div class="text-5xl mb-3">📦</div><div class="text-sm">Không có dữ liệu đặt hàng</div></div>
                            </div>
                ...
                <template v-if="viewMode === 'report'">
                    ...
                    <div class="p-6 flex justify-center bg-[#e8e8e8] min-h-[calc(100vh-120px)]">
                        <div class="bg-white shadow-lg border border-gray-300 w-full p-10 print:shadow-none print:border-none print:p-0 print:max-w-full"
                            ...
                            <h1 class="text-lg font-bold text-center mb-1">{{ chartData?.title || 'Báo cáo đặt hàng' }}</h1>
                            ...
                            <table class="w-full border-collapse" style="font-size:0.85em">
                                <thead>
                                    <tr class="bg-blue-600 text-white">
                                        <th class="px-3 py-2 text-left font-semibold border border-blue-700">{{ concern === 'product' ? 'Hàng hóa' : concern === 'status' ? 'Trạng thái' : 'Thời gian' }}</th>
