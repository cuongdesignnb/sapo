    ...
    <AppLayout>
        <template #sidebar>
            ...
            <div class="px-3 py-4 border-b border-gray-200">
                ...
                <div class="flex flex-wrap gap-2 text-[12px]">
                    <div v-for="status in statusFilters" :key="status" class="bg-green-600 text-white px-2 py-1 rounded flex items-center gap-1 cursor-pointer">
                        {{ formatStatus(status) }} <span @click.stop="removeStatusFilter(status)" class="pl-1 border-l border-green-400 font-bold hover:text-gray-200 ml-1">&times;</span>
                    </div>
                </div>
                <div class="mt-2 space-y-1">
                    <label v-for="s in allStatuses" :key="s.value" class="flex items-center gap-2 cursor-pointer text-sm text-gray-700">
                        <input type="checkbox" :value="s.value" v-model="statusFilters" class="rounded border-gray-300 text-green-600 focus:ring-green-500 w-4 h-4"> {{ s.label }}
                    </label>
                </div>
            </div>
            ...
            <div class="px-3 py-4 border-b border-gray-200">
                ...
                <div class="space-y-2 text-sm text-gray-700">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" v-model="dateFilter" value="this_month" class="text-green-600 focus:ring-green-500 w-4 h-4"> Tháng này
                    ...
                    <label class="flex items-center gap-2 cursor-pointer text-gray-500">
                        <input type="radio" v-model="dateFilter" value="custom" class="text-green-600 focus:ring-green-500 w-4 h-4"> Tùy chỉnh
            ...
            <div class="px-3 py-4 border-b border-gray-200">
                ...
                <input type="text" placeholder="Chọn nhà cung cấp" class="w-full border border-gray-300 rounded p-1.5 text-sm outline-none text-gray-700 bg-gray-50">
            </div>
        ...
        <div class="bg-white h-full flex flex-col pt-3">
            <div class="flex items-center justify-between px-4 pb-3 border-b border-gray-200">
                ...
                <div class="flex-1 max-w-[400px] ml-6 relative">
                    ...
                    <input type="text" v-model="search" placeholder="Theo mã phiếu nhập hàng, mã đặt hàng, ncc" class="w-full pl-9 pr-8 py-1.5 focus:outline-none border border-gray-300 rounded text-sm placeholder-gray-400">
                ...
                <div class="flex gap-2 ml-auto">
                    <Link href="/purchases/create" class="bg-white text-green-600 border border-green-600 px-3 py-1.5 text-sm font-medium rounded hover:bg-green-50 transition flex items-center gap-1">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg> Nhập hàng
                    </Link>
                        ...
                        <tr v-if="purchases.data.length === 0">
                            <td colspan="9" class="p-16 text-center text-gray-500">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p class="text-[13px]">Không tìm thấy phiếu nhập hàng nào phù hợp trong tháng này.</p>
                                </div>
                            </td>
                        </tr>
                        <template v-for="order in purchases.data" :key="order.id">
                            <tr @click="toggleExpand(order.id)" class="hover:bg-blue-50/40 cursor-pointer transition-colors" :class="{'bg-[#f4f7fe]': isExpanded(order.id), 'border-l-2 border-l-green-500': isExpanded(order.id)}">
                                ...
                                <td class="px-2 py-2 text-blue-600 font-medium">{{ order.code }}</td>
                                ...
                                <td class="px-4 py-2 text-right font-medium">{{ formatCurrency(order.total_amount) }}</td>
                                <td class="px-4 py-2 text-right font-medium">{{ formatCurrency(order.total_amount - order.discount) }}</td>
                                <td class="px-4 py-2 text-right font-medium text-green-600">{{ formatCurrency(order.paid_amount) }}</td>
                                <td class="px-4 py-2 text-center">
                                    <span class="inline-block px-2 text-[11px] py-0.5 rounded border font-medium bg-green-50 text-green-700 border-green-200">
                                        {{ order.status === 'completed' ? 'Hoàn thành' : 'Phiếu tạm' }}
                                    </span>
                                </td>
                            </tr>
                                ...
                                <td colspan="9" class="p-0 border-0 bg-white shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)]">
                                     <div class="p-4 flex gap-6 w-full border-t border-blue-100">
                                        ...
                                        <div class="w-80 border-l border-gray-200 pl-4 py-2 space-y-2 text-[13.5px]">
                                            <div class="flex justify-between items-center"><span class="text-gray-500">Mã phiếu nhập:</span> <strong>{{ order.code }}</strong></div>
                                            <div class="flex justify-between items-center"><span class="text-gray-500">Thời gian:</span> <span>{{ new Date(order.created_at).toLocaleString('vi-VN') }}</span></div>
                                            <div class="flex justify-between items-center"><span class="text-gray-500">Trạng thái:</span> <span class="font-bold text-green-600">Hoàn thành</span></div>
                                            <div class="flex justify-between items-center"><span class="text-gray-500">Ghi chú:</span> <span>{{ order.note || 'Không' }}</span></div>
                                            <div class="border-t border-gray-200 pt-3 mt-3 flex gap-2">
                                                <button class="bg-blue-600 text-white px-4 py-1.5 rounded font-medium hover:bg-blue-700 w-full">Thanh toán</button>
            ...
            <div class="flex items-center justify-between p-3 border-t border-gray-200 bg-gray-50/50 text-sm flex-shrink-0">
                <div class="text-gray-600">
                    Hiển thị từ <span class="font-bold">{{ purchases.from || 0 }}</span> đến <span class="font-bold">{{ purchases.to || 0 }}</span> trong tổng số <span class="font-bold">{{ purchases.total || 0 }}</span> phiếu
                </div>
                <!-- Pagination -->
