...
const loadPaymentHistory = async (invoiceId) => {
    ...
    try {
        const { data } = await axios.get(`/invoices/${invoiceId}/payment-history`);
...
const updateFilters = () => {
    ...
    searchTimeout = setTimeout(() => {
        router.get('/invoices', { search: search.value }, { preserveState: true, replace: true });
    }, 500);
};
...
const printInvoice = (invoice) => {
    window.open(`/invoices/${invoice.id}/print`, '_blank', 'width=400,height=600');
};
</script>
...
<template>
    ...
    <AppLayout>
        <template #sidebar>
            ...
            <!-- LOẠI HÓA ĐƠN -->
            <div class="px-3 py-4 border-b border-gray-200">
                <label class="block text-[13px] font-bold text-gray-800 mb-2">Loại hóa đơn</label>
                <div class="space-y-1.5">
                    <label class="flex items-center gap-2 cursor-pointer text-[13px] text-gray-700">
                        <input type="checkbox" checked class="rounded border-gray-300 text-blue-600 focus:ring-blue-500 w-4 h-4"> Không giao hàng
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer text-[13px] text-gray-700">
                        <input type="checkbox" checked class="rounded border-gray-300 text-blue-600 focus:ring-blue-500 w-4 h-4"> Giao hàng
                    </label>
                </div>
            </div>
            ...
            <!-- Lọc TRẠNG THÁI -->
            <div class="px-3 py-4 border-b border-gray-200">
                <label class="block text-[13px] font-bold text-gray-800 mb-2">Trạng thái hóa đơn</label>
                <div class="space-y-1.5">
                    <label class="flex items-center gap-2 cursor-pointer text-[13px] text-gray-700">
                        <input type="checkbox" checked class="rounded border-gray-300 text-blue-600 focus:ring-blue-500 w-4 h-4"> Đang xử lý
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer text-[13px] text-gray-700">
                        <input type="checkbox" checked class="rounded border-gray-300 text-blue-600 focus:ring-blue-500 w-4 h-4"> Hoàn thành
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer text-[13px] text-gray-700">
                        <input type="checkbox" class="rounded border-gray-300 text-blue-600 focus:ring-blue-500 w-4 h-4"> Không giao được
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer text-[13px] text-gray-700">
            ...
            <div class="px-3 py-4 border-b border-gray-200">
                <label class="block text-[13px] font-bold text-gray-800 mb-2">Trạng thái giao hàng</label>
                <select class="w-full border border-gray-300 rounded p-1.5 text-[13px] outline-none text-gray-500">
                    <option value="">Chọn trạng thái</option>
                </select>
            </div>
            
            <div class="px-3 py-4 border-b border-gray-200">
                <label class="block text-[13px] font-bold text-gray-800 mb-2">Đối tác giao hàng</label>
            </div>
        </template>
        ...
        <div class="bg-white h-full flex flex-col pt-3">
            <div class="flex items-center justify-between px-4 pb-3 border-b border-gray-200">
                ...
                <div class="flex-1 max-w-[400px] ml-6 relative">
                    ...
                    <input type="text" v-model="search" placeholder="Theo mã hóa đơn, mã KH, tên KH, sđt..." class="w-full pl-9 pr-8 py-1.5 focus:outline-none border border-gray-300 rounded text-sm placeholder-gray-400">
                </div>

                <div class="flex gap-2 ml-auto">
                    <Link href="/orders/create" class="bg-blue-600 text-white px-3 py-1.5 text-sm font-medium rounded hover:bg-blue-700 transition flex items-center gap-1">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg> Tạo mới
                    </Link>
                ...
                <table class="w-full text-[13px] text-left whitespace-nowrap bg-white">
                    <thead class="font-bold text-gray-700 bg-[#f4f6f8] border-b border-gray-200 sticky top-0 z-10 shadow-sm">
                        <tr>
                            ...
                            <th class="px-4 py-2 text-right">Tổng sau giảm giá</th>
                        ...
                        <tr v-if="invoices.data.length === 0">
                            <td colspan="8" class="p-16 text-center text-gray-500">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p class="text-[14px]">Không tìm thấy hóa đơn nào phù hợp.</p>
                                </div>
                            </td>
                        </tr>
                        <template v-for="invoice in invoices.data" :key="invoice.id">
                            <tr @click="toggleExpand(invoice.id)" class="hover:bg-blue-50/40 cursor-pointer transition-colors" :class="{'bg-[#f4f7fe]': isExpanded(invoice.id), 'border-l-2 border-l-blue-500': isExpanded(invoice.id)}">
                                <td class="px-3 py-2 text-center" @click.stop><input type="checkbox" class="rounded border-gray-300"></td>
                                <td class="px-2 py-2 font-medium" :class="isExpanded(invoice.id) ? 'text-gray-900' : 'text-blue-600'">{{ invoice.code }}</td>
                                <td class="px-2 py-2">{{ new Date(invoice.created_at).toLocaleString('vi-VN', {day:'2-digit', month:'2-digit', year:'numeric', hour:'2-digit', minute:'2-digit'}) }}</td>
                                <td class="px-2 py-2">{{ invoice.customer?.name || 'Khách lẻ' }}</td>
                                <td class="px-4 py-2 text-right">{{ formatCurrency(invoice.subtotal) }}</td>
                                <td class="px-4 py-2 text-right">{{ formatCurrency(invoice.discount) }}</td>
                                <td class="px-4 py-2 text-right font-medium text-blue-600">{{ formatCurrency(invoice.total) }}</td>
                                <td class="px-4 py-2 text-right">{{ formatCurrency(invoice.customer_paid) }}</td>
                            </tr>
                            <tr v-if="isExpanded(invoice.id)" class="border-b-4 border-blue-50">
                                 <td colspan="8" class="p-0 border-0 bg-white shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)]">
                                     <div class="px-6 py-4 w-full border-t border-blue-100 flex flex-col pt-0">
                                         ...
                                         <div class="flex text-[13.5px] font-semibold text-gray-600 border-b border-gray-200 sticky top-0 bg-white z-0 pt-2 mb-4">
                                             <button @click="setInvoiceTab(invoice.id, 'info')" :class="getInvoiceTab(invoice.id) === 'info' ? 'border-b-2 border-blue-600 text-blue-600' : 'hover:text-blue-600'" class="px-4 pb-2">Thông tin</button>
                                             <button @click="setInvoiceTab(invoice.id, 'payment')" :class="getInvoiceTab(invoice.id) === 'payment' ? 'border-b-2 border-blue-600 text-blue-600' : 'hover:text-blue-600'" class="px-4 pb-2">Lịch sử thanh toán</button>
                                         </div>

                                         <!-- Header Info -->
                                         <div v-if="getInvoiceTab(invoice.id) === 'info'">
                                         <div class="flex items-center gap-2 mb-4">
                                            <h2 class="text-[17px] font-bold text-gray-800">{{ invoice.customer?.name || 'A Dũng Kiều Mai' }}</h2>
                                            <svg class="w-4 h-4 text-gray-400 cursor-pointer" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg>
                                            <span class="text-gray-500 text-[13px] ml-1 flex items-center gap-2">
                                                {{ invoice.code }}
                                                <span class="bg-green-100 text-green-700 px-2 py-0.5 rounded textxs font-medium">{{ invoice.status || 'Hoàn thành' }}</span>
                                            </span>
                                         ...
                                         <div class="flex flex-col gap-6">
                                             ...
                                             <div class="grid grid-cols-3 gap-x-12 gap-y-3 text-[13.5px] text-gray-700 w-full mb-2">
                                                 <div class="flex items-center">
                                                     ...
                                                     <span class="text-gray-800">{{ invoice.created_by_name || 'Trần Văn Tiến' }}</span>
                                                 </div>
                                                 <div class="flex items-center">
                                                     <span class="text-gray-400 w-24">Người bán:</span>
                                                     <select class="border border-gray-300 rounded px-2 py-0.5 outline-none flex-1">
                                                         <option>{{ invoice.seller_name || 'Trần Văn Tiến' }}</option>
                                                     </select>
                                                 </div>
                                                 <div class="flex items-center justify-end">
                                                     <span class="text-gray-400 w-24">Ngày bán:</span>
                                                     <div class="flex items-center border border-gray-300 rounded px-2 py-0.5 w-[160px] bg-white">
                                                         <span class="flex-1">{{ new Date(invoice.created_at).toLocaleString('vi-VN', {day:'2-digit', month:'2-digit', year:'numeric', hour:'2-digit', minute:'2-digit'}) }}</span>
                                                 ...
                                                 <div class="flex items-center">
                                                     ...
                                                     <select class="border border-gray-300 rounded px-2 py-0.5 outline-none flex-1">
                                                         <option>{{ invoice.sales_channel || 'Bán trực tiếp' }}</option>
                                                     </select>
                                                 </div>
                                                 <div class="flex items-center">
                                                     <span class="text-gray-400 w-24">Bảng giá:</span>
                                                     <span class="text-gray-800">{{ invoice.price_book_name || 'Bảng giá chung' }}</span>
                                                 </div>
                                                 <div></div>
                                             </div>
                                                 ...
                                                 <table class="w-full text-[13.5px]">
                                                     <thead class="bg-white border-b border-gray-200">
                                                         <tr>
                                                             ...
                                                             <th class="px-4 py-3 text-right font-bold text-gray-800 w-32 border-r border-gray-100">Đơn giá</th>
                                                             <th class="px-4 py-3 text-right font-bold text-gray-800 w-32 border-r border-gray-100">Giảm giá</th>
                                                             <th class="px-4 py-3 text-right font-bold text-gray-800 w-32 border-r border-gray-100">Giá bán</th>
                                                     ...
                                                     <tbody class="divide-y divide-gray-100">
                                                         <tr v-for="item in invoice.items" :key="item.id">
                                                             ...
                                                             <td class="px-4 py-3">
                                                                 <div class="text-gray-800 flex items-center gap-1">{{ item.product?.name }} <svg class="w-3.5 h-3.5 text-blue-500" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path></svg></div>
                                                             ...
                                                             <td class="px-4 py-3 text-right text-gray-800">{{ formatCurrency(item.price - (item.discount || 0)) }}</td>
                                             ...
                                             <div class="flex gap-8 mb-4 min-h-[100px]">
                                                ...
                                                <div class="w-[40%] text-[13.5px]">
                                                    <div class="flex justify-between py-1.5 text-gray-500">
                                                        <span>Tổng tiền hàng ({{ invoice.items?.length || 0 }})</span>
                                                        <span class="text-gray-800 font-medium">{{ formatCurrency(invoice.subtotal) }}</span>
                                                    </div>
                                                    <div class="flex justify-between py-1.5 text-gray-500">
                                                        <span>Giảm giá hóa đơn</span>
                                                        <span class="text-gray-800 font-medium">{{ formatCurrency(invoice.discount) }}</span>
                                                    </div>
                                                    <div class="flex justify-between py-1.5 text-gray-500">
                                                        <span>Khách cần trả</span>
                                                        <span class="text-gray-800 font-medium">{{ formatCurrency(invoice.total) }}</span>
                                                    </div>
                                                    <div class="flex justify-between py-1.5 text-gray-500">
                                                        <span>Khách đã trả</span>
                                                        <span class="text-gray-800 font-medium">{{ formatCurrency(invoice.customer_paid) }}</span>
                                                    </div>
                                                </div>
                                             </div>
                                             ...
                                             <div class="flex justify-between border-t border-gray-300 pt-4 pb-2 text-[13px]">
                                                <div class="flex gap-2">
                                                    <button @click.stop="cancelInvoice(invoice)" class="bg-white border border-gray-300 px-3 py-1.5 rounded text-gray-700 hover:bg-gray-50 flex items-center gap-1.5 font-medium">
                                                ...
                                                <div class="flex gap-2">
                                                    <Link :href="`/orders/create?invoice_id=${invoice.id}`" class="bg-[#0070f4] text-white px-4 py-1.5 rounded font-medium hover:bg-blue-600 flex items-center gap-1.5">
                                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                                        Chỉnh sửa
                                                    </Link>
                                                    <Link :href="`/orders/create?action=return&invoice_id=${invoice.id}`" class="bg-white border border-gray-300 px-3 py-1.5 rounded text-gray-700 hover:bg-gray-50 flex items-center gap-1.5 font-medium">
                                                    ...
                                                    <button @click.stop="printInvoice(invoice)" class="bg-white border border-gray-300 px-3 py-1.5 rounded text-gray-700 hover:bg-gray-50 flex items-center gap-1.5 font-medium">
                                         ...
                                         <!-- Payment History Tab -->
                                         <div v-if="getInvoiceTab(invoice.id) === 'payment'">
                                             <div v-if="paymentLoading[invoice.id]" class="text-center py-8 text-gray-400">Đang tải...</div>
                                             <div v-else-if="paymentHistoryData[invoice.id]">
                                                 <div v-if="!paymentHistoryData[invoice.id].payments?.length" class="text-center py-8 text-gray-400">Không có lịch sử thanh toán</div>
                                                 <table v-else class="w-full text-[13px]">
                                                     ...
                                                     <tbody class="divide-y divide-gray-100">
                                                         <tr v-for="p in paymentHistoryData[invoice.id].payments" :key="p.id">
                                                             <td class="px-3 py-2 text-blue-600">{{ p.code }}</td>
                                                             <td class="px-3 py-2 text-gray-500">{{ new Date(p.created_at).toLocaleString('vi-VN') }}</td>
                                                             <td class="px-3 py-2">{{ p.method || 'Tiền mặt' }}</td>
                                                             <td class="px-3 py-2 text-right font-medium">{{ formatCurrency(p.amount) }}</td>
                                                             <td class="px-3 py-2 text-gray-500">{{ p.note || '' }}</td>
                                                         </tr>
                                                     </tbody>
                                                 </table>
                                             </div>
                                             <div v-else class="text-center py-8 text-gray-400">Đang tải...</div>
                                         </div>
            ...
            <div class="flex items-center justify-between p-3 border-t border-gray-200 bg-gray-50/50 text-sm flex-shrink-0">
                <div class="text-gray-600">
                    Hiển thị từ <span class="font-bold">{{ invoices.from || 0 }}</span> đến <span class="font-bold">{{ invoices.to || 0 }}</span> trong tổng số <span class="font-bold">{{ invoices.total || 0 }}</span> hóa đơn
                </div>
                <!-- Pagination -->
                <div class="flex gap-1" v-if="invoices.links && invoices.links.length > 3">
