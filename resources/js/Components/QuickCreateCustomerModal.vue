...
const resetForm = () => {
    Object.assign(form.value, {
        ...
        bank_name: "",
        bank_account: "",
        is_supplier: props.isSupplier,
        is_customer: !props.isSupplier,
    });
};
...
<template>
    <div
        v-if="show"
        class="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 pt-10 pb-10 font-sans"
        @click.self="close"
    >
        <div
        ...
        >
            ...
            >
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div class="border border-gray-200 rounded">
                        ...
                        >
                            <h3 class="font-bold text-gray-800">
                                Nhóm {{ entityLabel }}, ghi chú
                            </h3>
                        </div>
                        <div class="p-4 space-y-4">
                            <div>
                                <label class="block font-semibold mb-1"
                                    >Nhóm {{ entityLabel }}</label
                                >
                                <input
                    ...
                    <div
                        v-if="!isSupplier"
                        class="bg-gray-50 border border-gray-200 rounded px-4 py-4"
                    >
                        ...
                        >
                            <div class="flex items-center gap-6 mb-3">
                                ...
                                >
                                    <input
                                        ...
                                        value="existing"
                                        @change="
                                            form.link_existing_id =
                                                selectedExistingEntity?.id ||
                                                null
                                        "
                                        class="text-blue-600 focus:ring-blue-500 w-4 h-4"
                                    />
                            ...
                            <div
                                v-if="linkOption === 'existing'"
                                class="relative"
                            >
                                <div
                                    v-if="!selectedExistingEntity"
                                    class="relative"
                                >
                                    <svg
                                    ...
                                    <input
                                        v-model="searchExistingQuery"
                                        @focus="showExistingDropdown = true"
                                        type="text"
                                        class="w-full border border-gray-300 rounded pl-8 pr-3 py-1.5 focus:border-blue-500 outline-none"
                                        placeholder="Tìm tên nhà cung cấp..."
                                    />
                                </div>
                                <div
                                    v-else
                                    class="flex items-center justify-between border border-blue-200 bg-blue-50 rounded px-3 py-2"
                                >
                                    <div class="font-medium text-blue-700">
                                        {{ selectedExistingEntity.name }}
                                        <span
                                            class="text-gray-500 text-[12px] font-normal ml-2"
                                            >{{
                                                selectedExistingEntity.phone
                                            }}</span
                                        >
                                    </div>
                                    <button
                                        @click="removeExistingEntity"
                                        type="button"
                                ...
                                <div
                                    ...
                                    "
                                    class="absolute left-0 top-full mt-1 w-full bg-white border border-gray-200 shadow-xl rounded-sm z-[100] max-h-48 overflow-y-auto"
                                >
                                    <div
                                        v-if="isSearchingExisting"
                                    ...
                                    <div
                                        v-for="sup in existingResults"
                                        :key="sup.id"
                                        @mousedown.prevent="
                                            selectExistingEntity(sup)
                                        "
                                        class="p-2 cursor-pointer hover:bg-blue-50 border-b border-gray-100"
                                    >
                                        {{ sup.name }}
                    ...
                    >
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-[12px] text-gray-500 mt-0.5">
                                    Công nợ của nhà cung cấp và khách hàng sẽ
                                    được gộp với nhau
                                </p>
                            </div>
                            <label
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    ...
                                    <div
                                        v-for="cus in existingResults"
                                        :key="cus.id"
                                        @mousedown.prevent="
                                            selectExistingEntity(cus)
                                        "
                                        class="p-2 cursor-pointer hover:bg-blue-50 border-b border-gray-100"
                                    >
                                        {{ cus.name }}
            ...
            <div class="bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
                ...
                >
                    <h3 class="text-lg font-bold text-gray-800">
                        {{
                            dualRoleConfirm.type === "supplier"
                                ? "Xác nhận gộp khách hàng và nhà cung cấp"
                                : "Xác nhận gộp nhà cung cấp và khách hàng"
                        }}
                    </h3>
                    <button
                        @click="cancelDualRole"
                        class="text-gray-400 hover:text-gray-600"
                    >
                        <svg
                            class="w-5 h-5"
                ...
                <div class="px-6 py-5 space-y-4">
                    <div>
                        ...
                        <div class="text-sm text-gray-600 space-y-1">
                            ...
                            <div class="flex items-center gap-2 ml-4">
                                ...
                                Nợ hiện tại: 0
                                <template
                                    v-if="dualRoleConfirm.type === 'supplier'"
                                    >- Điểm: 0</template
                                >
                            </div>
                            <div class="mt-2">
                                ...
                                {{
                                    dualRoleConfirm.type === "supplier"
                                        ? "Nhà cung cấp"
                                        : "Khách hàng"
                                }}: <span class="text-gray-400">(tạo mới)</span>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="text-sm text-gray-600 space-y-1">
                            ...
                            <div class="flex items-center gap-2 ml-4">
                                <span
                                    class="w-2 h-2 rounded-full bg-green-500"
                                ></span>
                                Nhóm khách hàng: Có thể thay đổi do công nợ của
                                khách hàng thay đổi
                            </div>
                        </div>
                    </div>
                    ...
                    <div
                        class="text-sm text-gray-500 border-t border-gray-200 pt-3"
                    >
                        Hệ thống sẽ gộp công nợ, giao dịch khách hàng và nhà
                        cung cấp. Thông tin
                        <strong>Tên, Điện thoại, Địa chỉ...</strong> sẽ được
                        đồng bộ.
                    </div>
                ...
                >
                    <button
                        @click="confirmDualRole"
                        class="px-5 py-2 rounded text-white font-medium bg-blue-600 hover:bg-blue-700"
                    >
                        Đồng ý
                    </button>
                    <button
                        @click="cancelDualRole"
                        class="px-5 py-2 border border-gray-300 rounded text-gray-700 font-medium hover:bg-gray-50"
                    >
                        Bỏ qua
                    </button>
                </div>
