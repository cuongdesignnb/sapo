...
class AttendanceLogController extends Controller
{
    ...
    /**
     * Get list of device_user_ids that have no employee mapping
     */
    ...
    /**
     * Refresh employee mapping for existing logs
     */
    public function refreshMapping(Request $request)
    {
        ...
        // Use a single UPDATE JOIN when possible (MySQL/MariaDB) for speed.
...
SQL;
            ...
            // Auto recalculate timekeeping for impacted employees so the schedule UI updates.
            if ($updated > 0) {
                ...
                $min = $impacted?->min_punched_at ? \Carbon\Carbon::parse($impacted->min_punched_at) : null;
                $max = $impacted?->max_punched_at ? \Carbon\Carbon::parse($impacted->max_punched_at) : null;
        ...
        // Fallback: loop mapping (works across DB drivers).
        ...
        if (empty($employeeMap)) {
            return response()->json([
                'success' => false,
                'message' => 'Không có nhân viên nào có mã chấm công',
            ], 400);
        }
        ...
        foreach ($employeeMap as $code => $employeeId) {
            ...
            if ($deviceId) {
                $q->where('attendance_device_id', $deviceId);
            }

            if (!empty($deviceUserIds) && !in_array((string) $code, $deviceUserIds, true)) {
                continue;
            }
            ...
            if ($count > 0) {
                ...
                if ($range?->min_punched_at) {
                    $min = \Carbon\Carbon::parse($range->min_punched_at);
                    $max = \Carbon\Carbon::parse($range->max_punched_at);
                    if (!$minPunchedAt || $min->lessThan($minPunchedAt)) {
                        $minPunchedAt = $min;
                    }
                    if (!$maxPunchedAt || $max->greaterThan($maxPunchedAt)) {
                        $maxPunchedAt = $max;
                    }
                }
            }
